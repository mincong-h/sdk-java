 ...
 *
 * <p>In gRPC, a client application can directly call a method on a server application on a
 * different machine as if it were a local object, making it easier for you to create distributed
 ...
 * types. - On the server side, the server implements this interface and runs a gRPC server to
 * handle client calls. - On the client side, the client has a stub (referred to as just a client in
 * some languages) that provides the same methods as the server.
 *
 * <p>See https://www.grpc.io/docs/what-is-grpc/introduction/#overview
 */
public interface WorkflowServiceStubs
    extends ServiceStubs<
        /*
         This blocking stub is generated by the API contract
